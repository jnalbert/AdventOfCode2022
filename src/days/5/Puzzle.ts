...
export default class ConcretePuzzle extends Puzzle {
    ...
    private getRearrangements(): Rearrangement[] {
        return this.input.split('\n').map((line) => {
            ...
            return {amount: Number(sliptAtSpaces[1]), from: Number(sliptAtSpaces[3]) - 1 , to: Number(sliptAtSpaces[5]) - 1}
        })
    }
  ...
  public solveFirst(): string {
    ...
    rearrangements.forEach((rearrangement) => {
        ...
        const itemsToMove = fromRow.splice(fromRow.length - rearrangement.amount);
